...
double lightLocation[3] = {0.0,0,0};
double ambient = 0.2;
...
void set_object(){
  ...
  M3d_make_translation(a, -center[0], -center[1], -center[2]);
  M3d_make_z_rotation_cs(rot, cos(-45*M_PI/180), sin(-45*M_PI/180));
  M3d_mat_mult(a,rot,a);
  M3d_make_translation(b, center[0], center[1], center[2] + 3);
  M3d_mat_mult(a,b,a);
  

  M3d_mat_mult_points(x[0],y[0],z[0],a,x[0],y[0],z[0],numpoints[0]+1);

}

void s_input(double ss, double cs){
  ...
  M3d_make_translation(a, -center[0], -center[1], -center[2]);
  M3d_make_x_rotation_cs(rot, cs, ss);
  M3d_mat_mult(a,rot,a);
  M3d_make_y_rotation_cs(rot, cs, ss);
  M3d_mat_mult(a,rot,a);
  M3d_make_translation(b, center[0], center[1], center[2]);
  M3d_mat_mult(a,b,a);
...
void print_data(){
  ...
  for(i = 0; i < numpolys[0]; i++){
    a[0] = x[0][cont[0][i][1]] - x[0][cont[0][i][0]];
    a[1] = y[0][cont[0][i][1]] - y[0][cont[0][i][0]];
    a[2] = z[0][cont[0][i][1]] - z[0][cont[0][i][0]];

    b[0] = x[0][cont[0][i][2]] - x[0][cont[0][i][0]];
    b[1] = y[0][cont[0][i][2]] - y[0][cont[0][i][0]];
    b[2] = z[0][cont[0][i][2]] - z[0][cont[0][i][0]];
...
int main(int argc, char **argv){
  ...
  double ss = sin(sin(M_PI/4)*M_PI/180);
  double cs = cos(cos(M_PI/4)*M_PI/180);

  do{
    draw_all_object(1);
    input = G_wait_key();
    if(input == 's' || input == 'S'){s_input(ss,cs);}
